...
def pre_assign_bake_paths(objects: list, output_dir: str, model_name: str, bake_rvmat: bool) -> None:
    ...
    Manually set textures (bake_texture=False) are NOT cleared.
    """
    ...
    for sm in props.selection_mats:
        if not sm.bake_texture:
            # Manual textures are left as-is — exporter reads them directly
            continue
        # no_texture flag (added in 2.1.1) — skip selections without a texture
        if getattr(sm, "no_texture", False):
            continue
